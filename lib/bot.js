...
module.exports.Bot = (function() {
  ...
  var onStanza = function(stanza) {
    ...
    if (stanza.is('message') && stanza.attrs.type === 'groupchat') {
      ...
      this.emit('presence', fromJid, fromNick, stanza);
    }
  };
  ...
  // `onPresense(callback)`
  //
  // - `callback`: Function to be triggered: `function (user, stanza)`
  //   - `jid`: The full JID of the sender
  //   - `user`: The name of the person who joined the channel.
  //   - `stanza`: The full stanza from the presence packet
  Bot.prototype.onPresence = function (callback) {
    this.on('presence', function (jid, user, stanza) {
      callback(jid, user, stanza);
    });
  };
