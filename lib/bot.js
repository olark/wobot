...
module.exports.Bot = (function() {
  ...
  var onStanza = function(stanza) {
    ...
    if (stanza.is('message') && stanza.attrs.type === 'groupchat') {
      ...
      // If the user has gone away store their presence and return
      if (show && show.getText() === 'away') {
        presences[fromJid.toString()] = 'away';

        return;
      }

      // If the user is coming back from away delete their presence and return
      if (presences[fromJid.toString()] === 'away') {
        delete presences[fromJid.toString()];
  ...
  Bot.prototype.onJoin = function (condition, callback) {
    if (arguments.length === 1) {
      return this.on('join', condition);
    }
