...
module.exports.Bot = (function() {
  ...
  var onOnline = function() {
    ...
    this.keepalive = setInterval(function() {
      var emptyIq = new xmpp.Element('iq');
      self.jabber.send(emptyIq);
      self.emit('ping');
    }, 30000);
  ...
  var onStanza = function(stanza) {
    ...
    if (stanza.is('message') && stanza.attrs.type === 'groupchat') {
      ...
      var jid = nickNameToJid[fromNick];

      // Ignore our own messages
      if (fromNick === this.name) return;

      this.emit('message', fromChannel, fromNick, body, jid);
      ...
      var fromJid = new xmpp.JID(stanza.attrs.from);
      var nickname = jidToNickname[fromJid.bare().toString()];

      this.emit('privateMessage', fromJid.bare().toString(), body, nickname);
      ...
      // NOTE: this is designed to work with Hipchat, and may not work with
      //       other XMPP rooms

      var x = stanza.getChild('x', 'http://jabber.org/protocol/muc#user');
      if (!x) {
        // TODO: keep track of JID presence here.
        return;
      }else {
        ...
        // It looks like the event for joining and leaving
        // a room never includes the resource, but to be
        // safe use the bare() jid for now.

        var jid = new xmpp.JID(item.attrs.jid).bare().toString();
        var fromJid = new xmpp.JID(stanza.attrs.from);
        var fromChannel = fromJid.bare().toString();
        var fromNick = fromJid.resource;

        presences[fromChannel] = presences[fromChannel] || {};

        if(stanza.type == "unavailable"){
          // leaving a channel
          delete presences[fromChannel][jid];
          this.emit('leave', fromChannel, fromNick, role, jid);
          ...
          // Map nicknamesToJids
          nickNameToJid[fromNick] = jid;
          jidToNickname[jid] = fromNick;

          if(!presences[fromChannel][jid]){
            // Only send presence the first time we get presence for someone
            // until they leave the channel
            presences[fromChannel][jid] = +(new Date());
            this.emit('join', fromChannel, fromNick, role, jid);
          }
        }
      }
    }
  ...
  Bot.prototype.onMessage = function(condition, callback) {
    ...
    this.on('message', function(channel, from, message, jid) {
  ...
  Bot.prototype.onJoin = function (callback) {
    this.on('join', function (channel, user, role, jid) {
      callback.apply(this, arguments);
    });
  };

  // Emitted whenever a user leaves a channel the bot is in.
  //
  // `onLeave( callback)`
  //
  // - `callback`: Function to be triggered: `function (channel, user, role)`
  ...
  //   - `user`: The name of the person who joined the channel.
  //   - `role`: The role of the person who joined the channel.
  Bot.prototype.onLeave = function (callback) {
    this.on('leave', function (channel, user, role, jid) {
      callback.apply(this, arguments);
    });
  };
  ...
  Bot.prototype.onPrivateMessage = function(condition, callback) {
    ...
    this.on('privateMessage', function(from, message, nickname) {
      if (typeof condition === 'string' && message === condition) {
        callback.apply(this, arguments);
      } else if (condition instanceof RegExp) {
        var matches = message.match(condition);
