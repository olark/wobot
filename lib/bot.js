...
module.exports.Bot = (function() {
  ...
  var onStanza = function(stanza) {
    ...
    if (stanza.is('message') && stanza.attrs.type === 'groupchat') {
      ...
      this.emit('join', fromChannel, fromNick);
    }
  };
  ...
  // `onJoin(callback)`
  //
  // - `callback`: Function to be triggered: `function (roomJid, from, message)`
  //   - `channel`: The name of the room in which the message occured.
  //   - `user`: The name of the person who said the message.
  Bot.prototype.onJoin = function (callback) {
    this.on('join', function (channel, user) {
